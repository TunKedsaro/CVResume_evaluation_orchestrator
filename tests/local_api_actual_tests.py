# tests/local_api_actual_tests.py
"""
Local integration tests for CVResume_evaluation_orchestrator.
...
- Or run via pytest explicitly (NOT for CI):
    pytest -q tests/local_api_actual_tests.py -s
"""
...
def _extract_expectation(payload: Dict[str, Any], filename: str) -> Expectation:
    ...
    if isinstance(expect_obj, dict):
        ...
        allow_timeouts = expect_obj.get("allow_transient_downstream_timeouts", True)
        allow_timeouts = bool(allow_timeouts)
...
def _assert_success_contract(body: Dict[str, Any]) -> float:
    # New contract: on HTTP < 400, status must be "success"
    ...
    # Optional extra fallback (harmless, some services use this)
    if score is None:
...
def test_all_payloads_in_test_payloads_dir() -> None:
    ...
    for idx, path in enumerate(files, start=1):
        ...
        correlation_id = f"corr-local-payload-{path.stem}-{int(time.time())}"
...
if __name__ == "__main__":
    ...
    print("\nALL LOCAL API PAYLOAD TESTS PASSED ✅")
